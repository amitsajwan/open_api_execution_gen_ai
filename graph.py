...
"""
        ...
        if state.loop_counter >= 2:
            ...
            try:
                # Pass worker_llm directly to the helper function.
    ...
    def responder_node(state: BotState) -> Dict[str, Any]:
        ...
        if planner_last_decision == "executor":
            ...
            if execution_error:
                ...
                # Use LLM to synthesize execution results into a user-friendly summary.
        ...
        # These nodes (like identify_apis, describe_graph) set state.response or update state data.
        # Use LLM to generate user-friendly text for data outputs.
        elif planner_last_decision in ["identify_apis", "describe_graph", "get_graph_json", "generate_payloads", "generate_execution_graph"]:
             # Use LLM to synthesize a response based on the data generated by the previous node.
             ...
             if planner_last_decision == "identify_apis" and state.identified_apis:
                 ...
                 prompt += f"\nGenerated Payloads:\n```json\n{json.dumps(state.generated_payloads, indent=2)}\n```"
                  ...
                  # If graph was just generated, describe it.
                 ...
                 # Fallback to using the state.response if no specific data is found for the node type.
        ...
        # Handle responses for handle_unknown and handle_loop (which set state.response).
    ...
    for node_name in CORE_LOGIC_TO_RESPONDER:
         if node_name in tool_methods:
              # These nodes now route to the responder, which will use LLM for formatting
